...
if (-not ($AIRSPYONE_HOST_SOURCES | Test-Path)) {
    Get-PinnedSource https://github.com/airspy/airspyone_host $AIRSPYONE_HOST_SOURCES
}
...
if (-not ($AIRSPYONE_HOST_TARGET | Test-Path)) {
    ...
    cmake --build $AIRSPYONE_HOST_TARGET --config $CS_BUILD_TYPE --target install
}
...
if (-not ($SOAPYAIRSPY_SOURCES | Test-Path)) {
    Get-PinnedSource https://github.com/pothosware/SoapyAirspy $SOAPYAIRSPY_SOURCES
}
