...
if (-not ($HACKRF_SOURCES | Test-Path)) {
    Get-PinnedSource https://github.com/greatscottgadgets/hackrf $HACKRF_SOURCES
}
...
if (-not ($HACKRF_TARGET | Test-Path)) {
    ...
    cmake -B $HACKRF_TARGET -G $CS_GENERATOR -A $CS_BUILD_ARCH $HACKRF_SOURCES/host `
...
if (-not ($SOAPY_HACKRF_SOURCES | Test-Path)) {
    Get-PinnedSource https://github.com/pothosware/SoapyHackRF $SOAPY_HACKRF_SOURCES
}
...
if (-not ($SOAPY_HACKRF_TARGET | Test-Path)) {
    ...
    cmake -B $SOAPY_HACKRF_TARGET -G $CS_GENERATOR -A $CS_BUILD_ARCH $SOAPY_HACKRF_SOURCES `
